    ...
    extends Agent
{
                          ...
                          double beta)
    {
        ...
        else if(node.getDepth() >= this.getMaxDepth()){
            node.setUtilityValue(Heuristics.calculateHeuristicValue(node));
            return node;
        }
        else
        {
            List<Node> children = MoveOrderer.orderChildren(node.getChildren(), this.getMyPlayerType());

            for(Node child : children)
            {
                child.setUtilityValue(Heuristics.calculateHeuristicValue(child));
            }
            Node bestNode = null;
            if(node.getCurrentPlayerType() == this.getMyPlayerType())
            {
                double bestUtility = Double.NEGATIVE_INFINITY;
                for(Node child : children)
                {
                    Node evalNode = this.alphaBeta(child, alpha, beta);
                    double eval = evalNode.getUtilityValue();
                    if(eval >= bestUtility)
                    {
                        bestUtility = eval;
                        bestNode = child;
                ...
                bestNode.setUtilityValue(bestUtility);
                return bestNode;
            }
            else
            {
